"""Tests for the free energy difference workflow."""
...
from jfchemistry.workflows.free_energy_difference.workflow import (
    FreeEnergyDifferenceCalculation,
...
@dataclass
class _TrackingWorkflow(FreeEnergyDifferenceWorkflow):
    calls: list[tuple[str, str, int, int, bool]] = field(default_factory=list)
...
def test_workflow_dispatches_expected_six_jobs_plus_reducer() -> None:
    ...
    assert flow.name == "Free Energy Difference Workflow"
    assert len(flow.jobs) == EXPECTED_JOB_COUNT
    assert workflow.calls == [
        ("donor", "initial", 0, 1, True),
        ("donor", "final", 1, 1, True),
        ("donor", "final", 1, 1, False),
        ("acceptor", "initial", 0, 1, True),
        ("acceptor", "final", -1, 1, True),
        ("acceptor", "final", -1, 1, False),
    ]


def test_energy_reduction_nelson_four_point_formula() -> None:
    """Four-point free energy difference expression should match expected terms."""
    donor_term, acceptor_term, total_term = (
        FreeEnergyDifferenceCalculation._calculate_free_energy_differences(
    ...
    assert donor_term == pytest.approx(0.4)
    assert acceptor_term == pytest.approx(0.3)
    assert total_term == pytest.approx(0.7)
...
def test_output_contains_all_three_properties() -> None:
    ...
    workflow = FreeEnergyDifferenceWorkflow(optimizer=_ConstantEnergyMaker(energy_ev=0.0))
    donor = _single_hydrogen_molecule(charge=0, spin=1)
    acceptor = _single_hydrogen_molecule(charge=0, spin=1)
...
def test_missing_optimizer_raises_clear_error() -> None:
    ...
    workflow = FreeEnergyDifferenceWorkflow()
    donor = _single_hydrogen_molecule(charge=0, spin=1)
    acceptor = _single_hydrogen_molecule(charge=0, spin=1)
    with pytest.raises(
        ValueError, match=r"FreeEnergyDifferenceWorkflow requires an `optimizer` attribute\."
    ):
        workflow._build_flow(donor, acceptor)

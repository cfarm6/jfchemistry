...
@dataclass
class FairChemOptimizer(FairChemCalculator, ASEOptimizer):
    """Optimize molecular structures using FairChem neural network potential.

    Combines FairChem's fast neural network energy/force predictions with
    ASE optimization algorithms for efficient geometry optimization. Ideal
    for large molecular systems where speed is critical.

    Inherits all attributes from FairChemCalculator (model, charge, multiplicity)
    and ASEOptimizer (optimizer, fmax, steps).

    Attributes:
        name: Name of the optimizer (default: "FairChem Optimizer").
        Additional attributes inherited from FairChemCalculator and ASEOptimizer.

    Examples:
        >>> from ase.build import molecule # doctest: +SKIP
        >>> from pymatgen.core import Molecule # doctest: +SKIP
        >>> from jfchemistry.optimizers import FairChemOptimizer # doctest: +SKIP
        >>> molecule = Molecule.from_ase_atoms(molecule("CCH")) # doctest: +SKIP
        >>>
        >>> # Fast optimization for screening
        >>> opt_fast = FairChemOptimizer( # doctest: +SKIP
        ...
        >>>
        >>> # Tight optimization
        >>> opt_tight = FairChemOptimizer( # doctest: +SKIP
        ...     optimizer="LBFGS", # doctest: +SKIP
